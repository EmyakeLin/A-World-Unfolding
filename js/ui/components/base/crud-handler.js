...
export class CrudHandler {
  ...
  remove(setting, args) {
    ...
    return { error: `${this.entityType}不存在` };
  }
  ...
  applyChanges(entity, args) {
    for (const [key, value] of Object.entries(args)) {
      if (!['id', 'lore_path', 'node_id', 'char_id', 'scene_id', 'item_id', 'history_id', 'geo_id', 'edge_id'].includes(key) && value !== undefined) {
        entity[key] = value;
      }
    }
  }
...
export class WorldviewCrud extends CrudHandler {
  ...
  createEntity(id, args) {
    return { id, priority: args.priority || 99, name: args.name, tags: args.tags || ['自定义'], content: { description: args.content?.description || '', detail: args.content?.detail || '' } };
  }
  applyChanges(entity, args) {
...
export class EdgeCrud extends CrudHandler {
  ...
  createEntity(id, args) { return { id, subject: args.subject, relation: args.relation, object: args.object }; }
  applyChanges(entity, args) { if (args.relation) entity.relation = args.relation; }
...
export class CharacterCrud extends CrudHandler {
  ...
  createEntity(id, args) {
    return { id, name: args.name, default: { persona: args.persona || '', mbti: args.mbti || '', value: args.value || '', pursuit: args.pursuit || '', ability: args.ability || '', relationship: args.relationship || [], goal: args.goal || '', memory: { vital: args.memory?.vital || [], longTerm: args.memory?.longTerm || [], daily: args.memory?.daily || [] }, inventory: args.inventory || [], cognition: { worldview_blacklist: args.cognition?.worldview_blacklist || [], events: args.cognition?.events || [], items: args.cognition?.items || [], characters: args.cognition?.characters || {} } } };
  }
  applyChanges(entity, args) {
    if (args.name) entity.name = args.name;
...
export class ItemCrud extends CrudHandler {
  ...
  createEntity(id, args) { return { id, name: args.name, tags: args.tags || ['自定义'], default: { content: { description: args.content?.description || '', detail: args.content?.detail || '' }, ability: args.ability || '' } }; }
  applyChanges(entity, args) {
    if (args.name) entity.name = args.name;
    if (args.tags) entity.tags = args.tags;
    if (args.content?.description) entity.default.content.description = args.content.description;
...
export class HistoryCrud extends CrudHandler {
  ...
  createEntity(id, args) { return { id, time: args.time, content: { description: args.content?.description || '', detail: args.content?.detail || '' }, connection: args.connection || [] }; }
  applyChanges(entity, args) {
...
export class GeographyCrud extends CrudHandler {
  ...
  createEntity(id, args) { return { id, name: args.name, content: { description: args.content?.description || '' }, neighbors: args.neighbors || [], layer: args.layer || 1, father: args.father || null }; }
  applyChanges(entity, args) {
    if (args.name) entity.name = args.name;
    if (args.content?.description) entity.content.description = args.content.description;
