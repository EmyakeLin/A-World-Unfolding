...
// Deduction Level Popup - 使用 PopupBase + 延迟逻辑
...
// 延迟显示/隐藏逻辑
let dlShowTimeout = null;
let dlHideTimeout = null;
let isDlPinned = false;

window.openDeductionLevelPopup = (triggerEl) => deductionLevelPopup.show(triggerEl);
window.showDlPopup = (triggerEl) => {
    if (dlHideTimeout) { clearTimeout(dlHideTimeout); dlHideTimeout = null; }
    if (dlShowTimeout) clearTimeout(dlShowTimeout);
    if (isDlPinned) deductionLevelPopup.show(triggerEl);
    else dlShowTimeout = setTimeout(() => deductionLevelPopup.show(triggerEl), 200);
};
window.hideDlPopup = () => {
    if (dlShowTimeout) { clearTimeout(dlShowTimeout); dlShowTimeout = null; }
    if (isDlPinned) return;
    dlHideTimeout = setTimeout(() => deductionLevelPopup.hide(), 500);
};

// Popup 内部 mouseenter/mouseleave 延迟
if (deductionLevelPopup.el) {
    deductionLevelPopup.el.addEventListener('mouseenter', () => { if (dlHideTimeout) { clearTimeout(dlHideTimeout); dlHideTimeout = null; } });
    deductionLevelPopup.el.addEventListener('mouseleave', () => { window.hideDlPopup(); });
    deductionLevelPopup.el.querySelectorAll('.dl-popup-option').forEach(opt => {
        opt.addEventListener('click', (e) => {
            e.stopPropagation();
            isDlPinned = false;
            if (dlShowTimeout) clearTimeout(dlShowTimeout);
            if (dlHideTimeout) clearTimeout(dlHideTimeout);
            window.syncDeductionLevelToUI(opt.getAttribute('data-level'));
            deductionLevelPopup.hide();
        });
    });
}
